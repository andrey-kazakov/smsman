...
class OrderTest < ActiveSupport::TestCase
  ...
  test 'bulk order should create targets' do
    recipient_numbers = (1..16).to_a.map{ Faker::PhoneNumber.phone_number  }.join(' ')
    ...
    @order.save!
    @order.reload
    assert_equal 16, @order.targets.count
  end

  test 'individual order should create targets' do
    recipient_numbers_with_texts = ''
    16.times do
      recipient_numbers_with_texts += "#{Faker::PhoneNumber.phone_number} #{Faker::Lorem.pagagraph}\n"
    end
    @order = IndividualOrder.new(:name => Faker::Company.catch_phrase, :sender_number => Faker::PhoneNumber.phone_number, :recipient_numbers_with_text => recipient_numbers_with_texts)
    @user.orders << @order
